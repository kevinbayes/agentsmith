...
pub fn decode_jwt(config: Config, token: &str) -> Result<TokenData<DHClaims>, jsonwebtoken::errors::Error> {
    let secret_key = env::var("SHARED_KEY").unwrap_or_else(|_| config.dh.security.jwt.secret);
    let mut validation = Validation::new(Algorithm::HS256);
    validation.set_issuer(&["agentsmith"]);
    validation.set_audience(&["agentsmith"]);
    decode::<DHClaims>(token, &DecodingKey::from_secret(secret_key.as_bytes()), &validation)
}
...
pub fn generate_jwt(
    ...
    let secret_key = env::var("SHARED_SECRET").unwrap_or_else(|_| config.dh.security.jwt.secret);
