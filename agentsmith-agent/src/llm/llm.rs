...
impl LLMResult {
    ...
    pub fn from_grok(_response: &OpenAIGenerateResponse) -> Self {

        let response = _response.clone();
        let choice = if response.choices.len() > 0 {
            Some(response.choices[0].clone())
        } else {
            None
...
pub trait GenerateText {
    ...
    async fn generate(&self, prompt: &Prompt) -> agentsmith_common::error::error::SystemResult<LLMResult>;
}
