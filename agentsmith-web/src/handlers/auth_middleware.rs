...
use crate::common::error::{WebError, WebResult as CResult};
use axum::body::Body;
...
use agentsmith_common::error::error::ClientError;
...
pub async fn auth_middleware(
    ...
    let header = decode_header(token).map_err(|err|  "".to_string())?;

    let kid = header.kid.ok_or_else(||  "".to_string())?;
    let algo = header.alg;

    let jwk = state.cache.get_or_refresh(kid.as_str()).await.map_err(|e| {
        "".to_string()
    })?;
